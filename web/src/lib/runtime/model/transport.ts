// LocalAgentTransport — a serverless AI SDK UI ChatTransport. `useChat` calls sendMessages, which runs
// the in-browser @openai/agents SandboxAgent and translates its streamed run into a UIMessage stream via
// the Agents→AI SDK bridge.
//
// Human-in-the-loop: when a tool needs approval the run pauses and returns interruptions. The whole
// pause→approve→resume loop is wrapped in one createUIMessageStream so a single useChat turn keeps
// streaming — each run's chunks are merged in, and between runs the user's approve/reject decision (via
// the approval registry) gates resuming from result.state.
...
export class LocalAgentTransport implements ChatTransport<UIMessage> {
  async sendMessages({ messages, abortSignal }: Parameters<ChatTransport<UIMessage>["sendMessages"]>[0]): Promise<ReadableStream<UIMessageChunk>> {
    ...
    // callModelInputFilter trims old/oversized tool outputs before each model call so a small local
    // model's context window survives long shell/search/file results.
    ...
    return createUIMessageStream({
      ...
      execute: async ({ writer }: any) => {
        ...
        // Aggregate token usage for this turn; RunContext.usage is cumulative in result.state.
